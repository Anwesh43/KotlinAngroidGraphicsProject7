package com.example.doublearcmoveupview
...
class DoubleArcMoveUpView(ctx : Context) : View(ctx) {
    ...
    data class DAMUNode(var i : Int = 0, val state : State = State()) {

        private var next : DAMUNode? = null
        private var prev : DAMUNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawDAMUNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : DAMUNode {
            var curr : DAMUNode? = prev
            if (dir === 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class DoubleArcMoveUp(var i : Int) {

        private var curr : DAMUNode = DAMUNode(0)
