package com.example.stretchlinefullcircleview
...
fun Canvas.drawStretchLineFullCircle(scale : Float, w : Float, h : Float, paint : Paint) {
    ...
    drawXY(w / 2, h / 2 + (h / 2) * dsc(4)) {
        ...
        for (j in 0..1) {
            drawXY(0f, 0f) {
                rotate(-rot * (1 - 2 * j) * dsc(3))
                drawXY(0f, 0f) {
                    rotate(rot * (1 - 2 * j) * (dsc(1)))
...
fun Canvas.drawSLFCNode(i : Int, scale : Float, paint : Paint) {
...
class StretchLineFullCircleView(ctx : Context) : View(ctx) {
    ...
    data class StretchLineFullCircle(var i : Int) {

        private var curr : SLFCNode = SLFCNode(0)
