...
class BiLineBentArcView(ctx : Context) : View(ctx) {
    ...
    data class BLBANode(var i : Int = 0, val state : State = State()) {

        private var next : BLBANode? = null
        private var prev : BLBANode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawBLBANode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : BLBANode {
            var curr : BLBANode? = prev
            if (dir === 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class BiLineBentArc(var i : Int) {

        private var curr : BLBANode = BLBANode(0)
    ...
    data class Renderer(var view : BiLineBentArcView) {

        private val animator : Animator = Animator(view)
        private val paint : Paint = Paint(Paint.ANTI_ALIAS_FLAG)
        private val blba : BiLineBentArc = BiLineBentArc(0)

        fun render(canvas : Canvas) {
            canvas.drawColor(backColor)
            blba.draw(canvas, paint)
            animator.animate {
                blba.update {
                    animator.stop()
                }
