package com.example.lineconcjoinarcview
...
class LineConcArcJoinView(ctx : Context) : View(ctx) {
    ...
    data class LCAJNode(var i : Int = 0, val state : State = State()) {

        private var next : LCAJNode? = null
        private var prev : LCAJNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawLCAJNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : LCAJNode {
            var curr : LCAJNode? = prev
            if (dir === 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class Renderer(var view : LineConcArcJoinView) {

        private val animator : Animator = Animator(view)
        private val paint : Paint = Paint(Paint.ANTI_ALIAS_FLAG)
        private val lcaj : LineConcArcJoin = LineConcArcJoin(0)

        fun render(canvas : Canvas) {
            canvas.drawColor(backColor)
            lcaj.draw(canvas, paint)
            animator.animate {
                lcaj.update {
                    animator.stop()
                }
