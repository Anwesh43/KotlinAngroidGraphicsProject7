package com.example.joinlinesemicircleview
...
fun Canvas.drawJoinLineSemiCircle(scale : Float, w : Float, h : Float, paint : Paint) {
...
fun Canvas.drawJLSCNode(i : Int, scale : Float, paint : Paint) {
...
class JoinLineSemiCircleView(ctx : Context) : View(ctx) {
    ...
    data class JLSCNode(var i : Int = 0, val state : State = State()) {

        private var next : JLSCNode? = null
        private var prev : JLSCNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawJLSCNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : JLSCNode {
            var curr : JLSCNode? = prev
            if (dir === 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class JoinLineSemiCircle(var i : Int) {

        private var curr : JLSCNode = JLSCNode(0)
