package com.example.linedropextendarcview
...
fun Canvas.drawLineDropExtendArc(scale : Float, w : Float, h : Float, paint : Paint) {
    val size : Float = Math.min(w, h) / sizeFactor
    val dsc : (Int) -> Float = {
        scale.divideScale(it, parts)
    }
    drawXY(w / 2, h / 2 - h * 0.5f * (1 - dsc(0) - dsc(4))) {
        rotate(rot * dsc(3))
...
fun Canvas.drawLDEANode(i : Int, scale : Float, paint : Paint) {
    ...
    drawLineDropExtendArc(scale, w, h, paint)
}

class LineDropExtendArcView(ctx : Context) : View(ctx) {
    ...
    data class LineDropExtendArc(var i : Int) {

        private var curr : LDEANode = LDEANode(0)
