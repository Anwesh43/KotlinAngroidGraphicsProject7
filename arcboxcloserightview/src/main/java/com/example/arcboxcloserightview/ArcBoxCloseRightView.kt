package com.example.arcboxcloserightview
...
class ArcBoxCloseRightView(ctx : Context) : View(ctx) {
    ...
    data class ArcBoxCloseRight(var i : Int) {

        private var curr : ABCRNode = ABCRNode(0)
    ...
    data class Renderer(var view : ArcBoxCloseRightView) {

        private val animator : Animator = Animator(view)
        private val abcr : ArcBoxCloseRight = ArcBoxCloseRight(0)
        private val paint : Paint = Paint(Paint.ANTI_ALIAS_FLAG)

        fun render(canvas : Canvas) {
            canvas.drawColor(backColor)
            abcr.draw(canvas, paint)
            animator.animate {
                abcr.update {
                    animator.stop()
                }
