package com.example.halfslightjoinarcview
...
fun Canvas.drawHalfSlightJoinArc(scale : Float, w : Float, h : Float, paint : Paint) {
    ...
    drawXY(w / 2 + (w / 2) * dsc(4), h / 2) {
        ...
        for (j in 0..1) {
            drawXY(0f, 0f) {
                scale(1f - 2 * j, 1f)
                drawXY(size / 2, 0f) {
                    drawLine(0f, 0f, -size * 0.25f * dsc(1 + j), 0f, paint)
                }
            }
        }
    }
...
fun Canvas.drawHSJANode(i : Int, scale : Float, paint : Paint) {
    ...
    drawHalfSlightJoinArc(scale, w, h, paint)
}

class HalfSlightJoinArcView(ctx : Context) : View(ctx) {
    ...
    data class HalfSlightJoinArc(var i : Int) {

        private var curr : HSJANode = HSJANode(0)
    ...
    data class Renderer(var view : HalfSlightJoinArcView) {

        private val animator : Animator = Animator(view)
        private val paint : Paint = Paint(Paint.ANTI_ALIAS_FLAG)
        private val hsja : HalfSlightJoinArc = HalfSlightJoinArc(0)

        fun render(canvas : Canvas) {
            canvas.drawColor(backColor)
            hsja.draw(canvas, paint)
            animator.animate {
                hsja.update {
                    animator.stop()
                }
