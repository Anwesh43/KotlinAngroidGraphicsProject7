package com.example.bentlinearcrightview
...
fun Canvas.drawBentLineArcRight(scale : Float, w : Float, h : Float, paint : Paint) {
    ...
    drawXY(w / 2 + (w / 2) * dsc(3), h / 2) {
        for (j in 0..1) {
            drawXY(0f, 0f) {
                rotate(sweep * (1 - dsc(1)) * (1f - 2 * j) + rot * dsc(2))
...
fun Canvas.drawBLARNode(i : Int, scale : Float, paint : Paint) {
    ...
    drawBentLineArcRight(scale, w, h, paint)
}

class BentLineArcRightView(ctx : Context) : View(ctx) {
    ...
    data class BLARNode(var i : Int = 0, val state : State = State()) {
    ...
    data class BentLineArcRight(var i : Int = 0, val state : State = State()) {
